...
Call log:
  - waiting for locator('button[data-test^="add-to-cart"]').nth(5)
...
```ts
  ...
  3  | 
  4  | class InventoryPage extends BasePage {
  5  |   constructor(page) {
  6  |     super(page);
  7  | 
  8  |     this.cartIcon = '.shopping_cart_link';
  9  |     this.inventoryContainer = '.inventory_container';
  10 |     this.inventoryItems = '.inventory_item';
  11 |   }
  12 | 
  13 |   async verifyInventoryLoaded() {
  14 |     return await this.isVisible(this.inventoryContainer);
  15 |   }
  16 | 
  17 |   // select three random items and add to cart
  18 |   async addRandomItemsToCart(count = 3) {
  19 |     const buttons = await this.page.locator('button[data-test^="add-to-cart"]').all();
  20 | 
  21 |     const shuffled = buttons.sort(() => 0.5 - Math.random());
  22 |     const selected = shuffled.slice(0, count);
  23 | 
  24 |     for (const btn of selected) {
> 25 |       await btn.click();
     |                 ^ Error: locator.click: Test timeout of 30000ms exceeded.
  26 |     }
  27 | 
  28 |     Logger.info(`${count} random items added to cart`);
  29 |   }
  30 | 
  31 |   async goToCart() {
  32 |     await this.clickElement(this.cartIcon);
  33 |   }
  34 | }
  35 | 
  36 | module.exports = { InventoryPage };
```